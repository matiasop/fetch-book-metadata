...
TO_BE_IGNORED: set = {
    "Mis recortes  ",
    "Unknown (Usuario de Microsoft Office)",
...
def parse_datetime(data: str):
    ...
    if "Añadido el" in data:
        # Spanish data
        day, month_name, year, timestring = (
            data.replace("de", "").split(",")[-1].split()
        )
        ...
        # English data
        data = rreplace(data, ",", "")
        english_month_name, day, year, timestring, midday = (
            data.split(",")[1:][0].strip().split()
        )
...
def get_quote_metadata(
    quote_dict: DefaultDict, books_metadata: dict
    ...
    for title in quote_dict:
        ...
        for q in quote_dict[title]:
            data = {
                ...
                'metadata': q['metadata'],
                'quote': q['quote'],
                'initial_pos': q['initial_pos'],
                'final_pos': q['final_pos'],
                'date': q['date'],
                'time': q['time'],
                'book_title': book_title,
                'author': author,
...
def main():
    ...
    quote_complete_dict, quote_list, books_metadata = get_quote_metadata(
        quote_dict, books_metadata
    )
